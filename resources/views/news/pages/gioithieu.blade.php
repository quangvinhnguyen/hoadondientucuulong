@extends('news.layout.single')

@section('content')
<section class="row">
	<article class="post column">
        <h1 class="post-title text-center">HÓA ĐƠN ĐIỆN TỬ</h1>
        <h4>Hoá đơn điện tử đang là công cụ để Cơ quan Thuế tăng cường quản lý chặt chẽ việc khai báo thuế, đồng thời cũng là biện pháp để doanh nghiệp giảm chi phí quản lý và lưu trữ hoá đơn.</h4>

        <h4>Hoá đơn điện tử đang trở thành một xu thế bởi những lợi ích sau:</h4>

        <h4>+ Giảm thiểu chi phí in ấn, lưu trữ và vận chuyển hoá đơn</h4>

        <h4>+ Dễ dàng truy xuất</h4>

        <iframe width="860" height="316" src="https://www.youtube.com/embed/6Ujjubv_G3Q" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
       
        <h4>Đáp ứng nhu cầu ngày càng gia tăng của các doanh nghiệp về hoá đơn điện tử, Visnam cung cấp phần mềm xuất hoá đơn điện tử tiện lợi, đơn giản và dễ sử dụng nhất cho người dùng.</h4>
</article>
</section>
@endsection
@section('js')
	<script type="text/javascript" src="http://maps.google.com/maps/api/js?sensor=false"></script>
    <script type="text/javascript" src="js/gmap3.min.js"></script>
@endsection